    ...
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) setQuery(''); }}>
      <PopoverTrigger asChild>
        <button
          ...
          className={cn(
            'hidden h-9 items-center gap-2 rounded-md border py-0 pl-1.5 pr-2.5 text-[13px] font-semibold transition-colors md:inline-flex',
            open
      ...
      <PopoverContent align="end" className="w-[296px] p-0" sideOffset={8}>
        ...
        <div className="px-3.5 pb-1 pt-2 font-mono text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">
          Redaktionsteam
        </div>
        <div className="max-h-[236px] overflow-y-auto px-1.5 pb-1.5">
          {options.length === 0 && (
            <div className="px-2.5 py-3 text-[13px] text-muted-foreground">Keine Treffer.</div>
          )}
          {options.map((m) => {
        ...
        {assignee && (
          <button
            ...
            disabled={patch.isPending}
            className="flex w-full items-center gap-2.5 border-t px-3.5 py-2.5 text-[13px] font-semibold text-red-600 transition-colors hover:bg-red-50"
          >
