 ...
 * SCORE_WEIGHTS (lib/shared/score-weights.json via lib/shared/constants).
 * Use after weight recalibration so the JS-computed scores in the DB stay in
 * lockstep with the declared weights.
 *
 * Seit dem .ts-Port (2026-08-31) importiert das Skript die kanonische Formel
 * `computeStoredPressScore` aus lib/shared/scoring.ts direkt — inklusive der
 * 4-Nachkommastellen-Speicher-Rundung. Kein Formel-Zwilling mehr.
 *
 * Sets updated_at = now() explicitly because there is no PG trigger on
 * publications (see memory: prod_haiku_drift.md).
 *
 * Usage:
 *   npm run recompute-press-scores                            # dry-run, prints diff stats
 *   npm run recompute-press-scores -- --apply                 # writes to local DB
 *   npm run recompute-press-scores -- --apply --target=prod
 */
import { connectDb, parseScriptArgs } from './lib/db.mjs';
import { SCORE_DIMENSIONS, SCORE_WEIGHTS, type ScoreDimension } from '@/lib/shared/constants';
...
async function main(): Promise<void> {
  ...
  type Row = { id: string; old_score: string | number } & Record<ScoreDimension, number | null>;
  ...
  function newScore(r: Row): number {
    const dims = Object.fromEntries(
      SCORE_DIMENSIONS.map((k) => [k, r[k] ?? 0]),
    ) as Record<ScoreDimension, number>;
    // Kanonische Formel + Speicher-Rundung (4 Nachkommastellen) aus
    // lib/shared/scoring.ts — identisch zu beiden Schreibpfaden.
  ...
  const q = (p: number) => sortedDelta[Math.floor((sortedDelta.length - 1) * p)];
