#!/usr/bin/env node
// Schema-drift guard. lib/server/db/schema.ts is hand-mirrored from the raw-SQL
// Supabase migrations (NOT db:introspect — it renames existing relations). That
// manual mirror is one-sided: adding a table in a migration but forgetting it
// in schema.ts compiles fine and only fails at runtime. This check fails CI if
// any table created by a migration (and not later dropped, and not a view) is
// missing its pgTable() in schema.ts.
//
...
const schemaPath = join(root, 'lib', 'server', 'db', 'schema.ts');
...
if (missing.length > 0) {
  ...
  console.error('\nAdd them to schema.ts manually (do NOT run db:introspect — it renames existing relations).');
  process.exit(1);
}
