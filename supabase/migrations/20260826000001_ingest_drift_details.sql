-- Drift-Belege im Ingest-Report: WELCHE Verknüpfungen ins Leere zeigen,
-- nicht nur wie viele.
--
-- ANLASS. Die Nacht-Reports tragen seit jeher `person_link_orphans` und
-- `orgunit_link_orphans` als blosse Zahlen. Damit laesst sich weder entscheiden,
-- ob eine Nacht schlimm war, noch dem Problem nachgehen: "13 Orphans" nennt
-- kein einziges betroffenes Objekt. Beim Nachsehen am 2026-08-26 fiel auf, dass
-- ueber 28 Tage 265 Personen von Junctions referenziert werden, die der Export
-- nie mitliefert -- sichtbar wurde das erst durch eine Auswertung der
-- Rohexport-Archive, nicht durch den Report.
--
-- Diese Migration aendert NUR die Berichterstattung. Zaehlungen, Upserts,
-- Deletes und der Rueckgabewert bleiben identisch; hinzu kommt der Schluessel
-- `drift_details` mit bis zu 50 Belegzeilen je Junction-Art. Der Deckel haelt
-- den Report auch bei einem Ausreisser klein (2026-08-22: 343 Orphans).
--
-- Basis ist die LIVE-Definition der Funktion (zuletzt geaendert von
-- 20260729000001_persons_delta_preserve_unsent_fields.sql), nicht die
-- Ursprungsmigration -- sonst faellt der Personen-Fix vom 29.07. zurueck.

CREATE OR REPLACE FUNCTION public.apply_publications_delta(p_payload jsonb, p_opts jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
  ...
  v_op_deleted           int := 0;
  v_op_orphans           int := 0;
  -- Drift-Belege: nicht nur WIE VIELE Verknüpfungen ins Leere zeigen, sondern
  -- WELCHE. Ohne das ist jede Drift-Warnung eine Zahl ohne Handhabe -- man sieht
  -- "13 Orphans" und kann nichts damit anfangen. Gedeckelt, damit ein
  -- Ausreisser-Lauf (2026-08-22: 343) den Report nicht sprengt.
  v_pp_orphan_rows       jsonb := '[]'::jsonb;
  v_op_orphan_rows       jsonb := '[]'::jsonb;
  v_drift_sample_limit   int := 50;
  v_ita_flipped          int := 0;
...
BEGIN
  ...
  --
  --     KEY-PRESENCE-SEMANTIK (seit 2026-07-29). Vorher schrieb dieser Upsert
  --     stur ALLE Spalten aus der Payload. Der OeAW-Export liefert Personensätze
  --     aber zeitweise auf `{uid, lastname}` eingedampft aus — dadurch wurden
  --     vollständige Stammdaten (Vorname, ORCID, Slug, Biografie, Portrait,
  --     Mail) mit NULL zugebügelt: real 158 von 3170 Personen, lautlos, Nacht
  --     für Nacht, ohne dass irgendein Alarm ansprang.
  --
  --     `jsonb_populate_record(p, row_json)` mischt den gelieferten Satz auf die
  --     BESTEHENDE Zeile und trennt damit exakt die beiden Fälle:
  --       * Schlüssel fehlt      ⇒ keine Aussage ⇒ Bestandswert bleibt stehen.
  --       * Schlüssel = null/""  ⇒ upstream geleert ⇒ wird weiterhin gelöscht.
  --     Das ist am Feed eindeutig unterscheidbar: bei vollständigen Sätzen
  --     schickt der Export jede Spalte mit (leer als ""), ein reduzierter Satz
  --     lässt sie ganz weg. Gegenstück im TS: normalizePerson gibt nur
  --     tatsächlich gelieferte Felder aus.
  --
  --     Kein Match im LEFT JOIN (= neue Person) ⇒ Basis ist NULL ⇒ fehlende
  --     Felder werden NULL, wie bisher. Das INSERT/ON-CONFLICT-Set darunter ist
  --     unverändert; es sieht durch die Vormischung nur bereits korrekte Werte.
  WITH raw AS (
    SELECT x AS row_json
    FROM jsonb_array_elements(COALESCE(v_up->'persons', '[]'::jsonb)) AS x
  ),
  src AS (
    SELECT
      m.*,
      -- member_type kommt als webdb_uid und ist KEINE persons-Spalte, muss also
      -- an jsonb_populate_record vorbei separat behandelt werden.
      (r.row_json ? 'member_type_webdb_uid')       AS has_member_type,
      (r.row_json->>'member_type_webdb_uid')::int  AS member_type_webdb_uid
    FROM raw r
    LEFT JOIN persons p ON p.webdb_uid = (r.row_json->>'webdb_uid')::int
    CROSS JOIN LATERAL jsonb_populate_record(p, r.row_json) AS m
  ),
  ins AS (
    INSERT INTO persons (
    ...
    SELECT
      ...
      s.research_fields, s.selected_publications,
      -- Nur neu auflösen, wenn der Feed das Feld überhaupt mitgeschickt hat;
      -- sonst den bestehenden FK behalten (kommt aus der Vormischung).
      CASE WHEN s.has_member_type THEN mt.id ELSE s.member_type_id END,
  ...
  WITH o AS (
    SELECT s.person_webdb_uid, s.publication_webdb_uid,
           (pe.id IS NULL) AS person_missing,
           (pu.id IS NULL) AS publication_missing
  ...
  n AS (SELECT count(*) AS c FROM o),
  smp AS (
    SELECT COALESCE(jsonb_agg(to_jsonb(t) ORDER BY t.publication_webdb_uid), '[]'::jsonb) AS j
    FROM (SELECT * FROM o ORDER BY publication_webdb_uid LIMIT v_drift_sample_limit) t
  )
  SELECT n.c, smp.j INTO v_pp_orphans, v_pp_orphan_rows FROM n, smp;
  ...
  WITH oo AS (
    SELECT s.orgunit_webdb_uid, s.publication_webdb_uid,
           (o.id IS NULL) AS orgunit_missing,
           (pu.id IS NULL) AS publication_missing
  ...
  n AS (SELECT count(*) AS c FROM oo),
  smp AS (
    SELECT COALESCE(jsonb_agg(to_jsonb(t) ORDER BY t.publication_webdb_uid), '[]'::jsonb) AS j
    FROM (SELECT * FROM oo ORDER BY publication_webdb_uid LIMIT v_drift_sample_limit) t
  )
  SELECT n.c, smp.j INTO v_op_orphans, v_op_orphan_rows FROM n, smp;
  ...
  v_report := jsonb_build_object(
    ...
    'drift_details', jsonb_build_object(
      'person_links', v_pp_orphan_rows,
      'orgunit_links', v_op_orphan_rows,
      'sample_limit', v_drift_sample_limit
    )
  );
