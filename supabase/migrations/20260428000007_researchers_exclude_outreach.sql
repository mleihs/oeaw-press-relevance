-- Researcher functions: add p_exclude_outreach boolean parameter (default true).
-- Excludes ÖAW-own outreach formats (currently: pub_type 'aufwändige Multimedia-Publikation'
-- — Pragmaticus/Hiccup podcast series & similar). These score high in
-- AI public_accessibility/storytelling rubric but are already-published owned
-- media, not press-target research. List can grow later via the same parameter.
--
-- Function signatures change → DROP first.

DROP FUNCTION IF EXISTS top_researchers(date, text, text, text[], boolean, boolean, boolean, numeric, int, boolean);
DROP FUNCTION IF EXISTS researcher_distribution(date, text, text, text[], boolean, boolean, boolean, numeric, int, boolean);
DROP FUNCTION IF EXISTS researcher_detail(uuid, date, boolean);
