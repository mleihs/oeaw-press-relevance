-- Add `citation` to the JSONB outputs of top_researchers and researcher_detail.
-- The client-side displayTitle() helper uses the citation to expand WebDB-truncated
-- titles (e.g. "Wissenschaftliche Zusammenfassung" → full subtitle from citation).
--
-- Function signatures don't change (top_pub / publications stay jsonb), so plain
-- CREATE OR REPLACE works — no DROP needed.
...
AS $$
  ...
  window_pubs AS MATERIALIZED (
    SELECT pp.person_id, p.id AS pub_id, p.title, p.haiku, p.citation,
  ...
  LEFT JOIN LATERAL (
    SELECT jsonb_build_object(
      'id', wp.pub_id, 'title', wp.title, 'haiku', wp.haiku,
      'citation', wp.citation, 'press_score', wp.press_score
    ) AS top_pub
    FROM window_pubs wp
...
AS $$
  ...
  window_pubs AS MATERIALIZED (
    SELECT pp.publication_id AS pub_id, p.title, p.haiku, p.citation,
           p.press_score, p.published_at,
  ...
  SELECT
    ...
    (
      SELECT jsonb_build_object(
        ...
        'top_pub',              (
          SELECT jsonb_build_object(
            'id', pub_id, 'title', title, 'haiku', haiku,
            'citation', citation, 'press_score', press_score
          )
