-- Personen-Upsert im Publications-Delta auf Key-Presence-Semantik umstellen.
--
-- ANLASS: der OeAW-Export liefert Personensätze zeitweise auf `{uid, lastname}`
-- eingedampft aus (beobachtet 2026-07-22 und durchgehend bis 2026-07-29; am
-- 29.07. waren 3686 von 3686 Personenzeilen so reduziert). Der bisherige Upsert
-- schrieb stur alle Spalten und überbügelte damit vollständige Stammdaten mit
-- NULL — real 158 von 3170 Personen (5 %), davon 145 an press-gescorten
-- Publikationen. Der Lauf meldete dabei jedes Mal `applied`; es gab keinen Alarm.
--
-- ÄNDERUNG: ausschließlich Block (c) der Funktion. `jsonb_populate_record` mischt
-- den gelieferten Satz auf die bestehende Zeile, sodass ein FEHLENDER Schlüssel
-- den Bestandswert behält, ein gelieferter `null`/`""` aber weiterhin löscht.
-- Alles andere ist byte-identisch aus 20260710000001 übernommen.
--
-- Gegenstück im TS: normalizePerson gibt nur tatsächlich gelieferte Felder aus.
-- Rollback: die Funktionsdefinition aus 20260710000001 erneut einspielen.
...
COMMENT ON FUNCTION apply_publications_delta IS
  'Wendet ein normalisiertes Publications-Delta (jsonb) atomar an: Deletes (Person hart/geschützt, Pub soft-archive), Upserts per webdb_uid, Junction-Upsert/Delete per Composite-Key mit INNER-JOIN-FK-Auflösung, scoped is_ita-Recompute, Bestands-Backfills, Cursor. Werte werden vorab in TS normalisiert (DOI single-sourced). Der PERSONEN-Upsert arbeitet key-presence-basiert (jsonb_populate_record auf die bestehende Zeile): ein im Feed fehlendes Feld behält den Bestandswert, ein geliefertes null/"" löscht ihn — Schutz gegen die zeitweise auf {uid,lastname} eingedampften OeAW-Personensätze. Ein SELECT-Aufruf ist all-or-nothing; Matview-Refresh macht der Aufrufer NACH Commit.';
