...
// session-pipeline.ts (doi-backfill) gemeinsam genutzt, damit Drift
 ...
 *
 * Hieß bis 2026-08-31 `cleanDoi` — umbenannt wegen Namenskollision mit
 * `cleanDoi` in lib/shared/doi-utils.ts, das ANDERE Semantik hat
 * (doi.org-/doi:-Prefix-Stripping + Validierung eines schon bekannten DOIs,
 * liefert null bei ungültig). Diese Variante hier putzt Extraktions-Rauschen
 * am Ende eines frisch gematchten DOI-Kandidaten weg und liefert immer
 * einen String.
 */
export function stripDoiNoise(s) {
...
export function extractDoiFromRow(row) {
  ...
  if (row.bibtex) {
    ...
    if (tagged && /^10\.\d{4,9}\//.test(tagged[1])) return stripDoiNoise(tagged[1]);
