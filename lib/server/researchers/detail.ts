...
import type { LoadedResearcherDetail, ResearcherDetail } from '@/lib/shared/researchers';
 ...
 * with either `person` or `stats` null (no usable detail). Otherwise the
 * `LoadedResearcherDetail` narrowing guarantees both fields non-null so
...
export async function getResearcherDetail(opts: {
...
}): Promise<LoadedResearcherDetail | null> {
  ...
  const row = rows[0];
  if (!row || !row.person || !row.stats) return null;
  return row as LoadedResearcherDetail;
}
